angular.module('rook', ['ui.bootstrap', 'LocalStorageModule']);
var RookGame = function ($scope, localStorageService) {

    var storedHands = localStorageService.get('hands');
    var storedPlayers = localStorageService.get('players');
    var storedNothing = localStorageService.get('foo');

    if (storedPlayers) {
        $scope.players = storedPlayers;
    }
    else {
        $scope.players = [
        { name: "John" },
    ...
    $scope.recalculateSums = function () {
        ...
        localStorageService.set('hands', JSON.stringify($scope.hands));
